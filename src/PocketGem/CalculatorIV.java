...
public class CalculatorIV {
    ...
    // +-*/ ^
    // we need to define a priority map
    // if current op has lower priority than before, we need to calculate before first
    // else (curr has high), we push into the stack, waiting for calculation
    // time O(n)
    // space O(n)
